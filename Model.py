...
class NetLayer:
    ...
    def __init__(self, inData, kind, inChannels, outChannels, filtSize=None, strides=None):
        if kind == "convolution":
            layer, weights, bias = convLayer(inData, filtSize, inChannels, outChannels)
        elif kind == "fully_connected":
            layer, weights, bias = fcLayer(inData, inChannels, outChannels)
...
def convLayer(din, filtSize, in_channels, out_channels, name=""):
    with tf.name_scope(name):
        ...
        with tf.name_scope('convlution'):
            convOut = tf.add(tf.nn.conv2d(din, w, stride=1, padding='SAME', name=name), b)
